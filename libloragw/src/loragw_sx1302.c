...
#define ARRAY_SIZE(a) (sizeof(a) / sizeof((a)[0]))
    ...
    #define CHECK_NULL(a)                 if(a==NULL){sx1303_log_debug("%s:%d: ERROR: NULL POINTER AS ARGUMENT\n", __FUNCTION__, __LINE__);return LGW_REG_ERROR;}
#else
...
const uint8_t agc_firmware_sx1250[8192] __attribute__((section(".rodata"))) = {
...
const uint8_t arb_firmware[8192] __attribute__((section(".rodata"))) = { 
...
int sx1302_init(const struct lgw_conf_ftime_s * ftime_context) {
    ...
    if (ftime_context->enable == true) {
        ...
        if (x != LGW_REG_SUCCESS) {
            DEBUG_MSG("ERROR: failed to get Chip Model ID\n");
            return LGW_REG_ERROR;
        }

        if (model_id != CHIP_MODEL_ID_SX1303) {
            DEBUG_PRINTF("ERROR: Fine Timestamping is not supported on this Chip Model ID 0x%02X\n", model_id);
            return LGW_REG_ERROR;
        }
    }
    x = timestamp_counter_mode(ftime_context->enable);
    if (x != LGW_REG_SUCCESS) {
        DEBUG_MSG("ERROR: failed to configure timestamp counter mode\n");
        return LGW_REG_ERROR;
    }

    x = sx1302_config_gpio();
    if (x != LGW_REG_SUCCESS) {
        DEBUG_MSG("ERROR: failed to configure sx1302 GPIOs\n");
        return LGW_REG_ERROR;
    }
...
int sx1302_radio_clock_select(uint8_t rf_chain) {
    ...
    if (err != LGW_REG_SUCCESS) {
        DEBUG_PRINTF("ERROR: failed to select radio clock for radio_%u\n", rf_chain);
        return LGW_REG_ERROR;
    }
...
int sx1302_radio_set_mode(uint8_t rf_chain, lgw_radio_type_t type) {
    ...
    if (err != LGW_REG_SUCCESS) {
        DEBUG_PRINTF("ERROR: failed to set mode for radio %u\n", rf_chain);
        return LGW_REG_ERROR;
    }
...
int sx1302_radio_calibrate(struct lgw_conf_rxrf_s * context_rf_chain, uint8_t clksrc, struct lgw_tx_gain_lut_s * txgain_lut) {
    ...
    for (i = 0; i < LGW_RF_CHAIN_NB; i++) {
        if (context_rf_chain[i].enable == true) {
            ...
            if (err != LGW_REG_SUCCESS) {
                DEBUG_PRINTF("ERROR: failed to set radio %d mode\n", i);
                return LGW_REG_ERROR;
            }
        }
    }
    DEBUG_MSG("sx1302_radio_clock_select\n");
    /* -- Select the radio which provides the clock to the sx1302 */
    err = sx1302_radio_clock_select(clksrc);
    if (err != LGW_REG_SUCCESS) {
        DEBUG_PRINTF("ERROR: failed to get select clock from radio %u\n", clksrc);
        return LGW_REG_ERROR;
    }

    DEBUG_MSG("Loading CAL fw\n");
    ...
    if ((context_rf_chain[clksrc].type == LGW_RADIO_TYPE_SX1257) ||
        ...
        if (err != LGW_REG_SUCCESS) {
            DEBUG_MSG("ERROR: Failed to load calibration fw\n");
            return LGW_REG_ERROR;
        }
        err = sx1302_cal_start(FW_VERSION_CAL, context_rf_chain, txgain_lut);
        if (err != LGW_REG_SUCCESS) {
            DEBUG_MSG("ERROR: radio calibration failed\n");
...
int sx1302_agc_load_firmware(const uint8_t *firmware) {
    ...
    if (err != LGW_REG_SUCCESS) {
        DEBUG_PRINTF("ERROR: Failed to take control over AGC MCU\n");
        return LGW_REG_ERROR;
    }

    /* Write AGC fw in AGC MEM */
    // err |= lgw_mem_wb(AGC_MEM_ADDR, firmware, MCU_FW_SIZE);
    err |= lgw_mem_wb(AGC_MEM_ADDR, agc_firmware_sx1250, MCU_FW_SIZE);
    ...
    */
    ...
    if (val != 0) {
        DEBUG_PRINTF("ERROR: Failed to load AGC fw: parity error check failed\n");
        return LGW_REG_ERROR;
    }
...
int sx1302_arb_load_firmware(const uint8_t *firmware) {
    ...
    err |= lgw_mem_wb(ARB_MEM_ADDR, arb_firmware, MCU_FW_SIZE);
...
int sx1302_tx_abort(uint8_t rf_chain) {
    ...
    if (err != LGW_REG_SUCCESS) {
        DEBUG_PRINTF("ERROR: Failed to stop TX trigger\n");
        return err;
    }
    ...
    do {
        ...
        if (timeout_check(tm_start, 1000) != 0) {
            DEBUG_PRINTF("ERROR: %s: TIMEOUT on TX abort\n", __FUNCTION__);
            return LGW_REG_ERROR;
        }
